...
public class StackInteractionZone : MonoBehaviour
{
    ...
    private void HandleInput(Inventory stackManager)
    {
        ItemStacker playerSource = GetMatchingPlayerStacker(stackManager, targetStacker.AcceptableType);
    ...
    private void HandleOutput(Inventory stackManager)
    {
        ItemStacker playerTarget = GetMatchingPlayerStacker(stackManager, targetStacker.AcceptableType);
    ...
    private ItemStacker GetMatchingPlayerStacker(Inventory manager, ItemType type)
    {
        if (manager.StoneStacker.AcceptableType == type) return manager.StoneStacker;
        if (manager.GoldStacker.AcceptableType == type) return manager.GoldStacker;
        if (manager.PotionStacker.AcceptableType == type) return manager.PotionStacker;
        return null;
    }
}